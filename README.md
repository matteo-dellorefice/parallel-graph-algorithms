...
### Generate a random sparse graph with a million vertices in under 300 milliseconds:

```
unsigned int seed = time(NULL);
unsigned int n_vertices = 1000000;
unsigned int edge_probability = 0.001;
// Coordinate graph representation
auto d_graph = pgra::device_graph_coo::create_random(seed, 
    n_vertices, edge_probability);
```
